...
class DespesaFuncionario {
	...
	update() {
		...
		this.db.prepare(query).run(this.id_funcionario, this.id_despesa, this.id);
	}
	...
	getDespesaForIdFuncionario() {
		...
		const query = `
        SELECT 
            d.id, d.descricao, c.categoria, d.valor, d.data, sd.status_despesa 
        FROM 
            tabDespesasFuncionarios AS df 
        INNER JOIN 
            tabDespesas AS d ON df.id_despesa = d.id
        INNER JOIN
            tabStatusDespesa AS sd ON d.status_despesa = sd.id
        INNER JOIN
